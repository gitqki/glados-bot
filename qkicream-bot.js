...
const helper = require("./classes/helper.class.js");
const bootstrap = require("./classes/bootstrap.class.js");
const Discord = require("discord.js");
const client = new Discord.Client();
new construct(); // constructor, not in use atm
...
client.on('message', msg => {
    ...
    if (msg.content.startsWith(CONFIG.prefix + "prefix")) {
        ...
        msg.delete().then(msg => console.log(gct() + ' - ' + msg.author.tag + ' changed Prefix to "' + newPrefix + '"'));
    ...
    if (msg.content.startsWith(CONFIG.hime + "play") || msg.content === CONFIG.hime + 'stop' || msg.content === CONFIG.hime + 'resume' ||
        ...
        // Delete message
        msg.delete()
            .then(msg => console.log(gct() + ' - Autodelete'))
            .catch(console.error);
    ...
    if (msg.content.startsWith(CONFIG.prefix + "clr")) {
        ...
        if (astr === '1') {
            msg.channel.fetchMessages({
                ...
                .then(messages => msg.channel.bulkDelete(2)).then(console.log(gct() + ' - Deleted message from: ' + msg.author.tag + '!'))
                .catch(console.error);
            setTimeout(() => {
                msg.channel.send("deleted messages: " + (astr)).then(console.log(gct() + ' - ' + astr + ' deleted message/s from: ' + msg.author.tag + '!'))
            }, 500);
        } else {
            astr = astr++ + 1;
            msg.channel.fetchMessages({
                ...
                .then(messages => msg.channel.bulkDelete(astr)).then(console.log(gct() + ' - Deleted message from: ' + msg.author.tag + '!'))
                .catch(console.error);
            setTimeout(() => {
                msg.channel.send("deleted messages: " + (astr-- - 1)).then(console.log(gct() + ' - ' + astr + ' deleted message/s from: ' + msg.author.tag + '!'))
            }, 500);
        }
    }
    ...
    if (msg.content === CONFIG.prefix + "commands") {
        ...
        msg.channel.send(
            ...
            CONFIG.format + '\n'
        ).then(msg => console.log(gct() + ' - Sent a reply to: ' + msg.author)).catch(console.error)
    }
});
